...
- **Project Manager client**: `create_resource` POSTs to `/projects/{id}/project_resources/{type}` with the `Idempotency-Key` header; `update_resource` PATCHes by `vendor_id`; `delete_resource` DELETEs by `vendor_id` (a `404` is treated as already-deleted). Finalize PATCHes `in_progress=False` — plus the spec's fields on an UPDATE — or DELETEs the record on a DELETE. **Confirm the provider actually exposes the DELETE**; the plugin doc lists only Create and Patch.
- **Engine-reported vendor id**: on engine success `RunEngineStep` reads the `final_vendor_id` output and, if present, records it in `state.step_results[RUN_ENGINE]`; `FinalizeResourceStep` then re-keys the record to it as it PATCHes (for a CREATE, off the run-id placeholder), leaving the record where it is when the run produced no output (reading the output is best-effort — a fetch error never fails a succeeded run).
