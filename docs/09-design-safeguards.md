...
- **Exactly-once side effects** — server-side idempotency keys `(run_id, step)`, stable across re-drives and retries (ServiceNow: `correlation_id`; Project Manager: `Idempotency-Key`), independent of any transport; the engine trigger instead uses an attempt-scoped key (Airflow: `dag_run_id`) so a retry starts a fresh engine run. Plus typed idempotency markers in `RunState` (`ticket`, `resource_configured`, `resource_finalized`, `ticket_closed`, …) so a re-driven step skips completed work.
