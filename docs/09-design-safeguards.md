...
- **No lost updates** — `version` optimistic concurrency on `WorkflowRun` (`StaleRunError`); on conflict the executor drops the lost save and another worker re-drives — it never clobbers. With the webhook removed there is a single writer path in normal operation; the conflict case is an overlapping re-drive after a lease expiry.
