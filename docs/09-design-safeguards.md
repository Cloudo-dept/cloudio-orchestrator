...
- **Exactly-once side effects** — server-side idempotency keys `(run_id, step)`, stable across re-drives and retries (ServiceNow: `correlation_id`; Project Manager: `Idempotency-Key`), independent of any transport; the engine trigger instead uses an attempt-scoped key (Airflow: `dag_run_id`) so a retry starts a fresh engine run. Plus typed idempotency markers in `RunState` (`ticket`, `resource_registered`, `resource_configured`, `ticket_closed`, …) so a re-driven step skips completed work.
...
- **Terminal failure, no rollback** — a step that exhausts retries ends the run at `FAILED` and escalates (Incident + RITM note); partially-created resources are left in place for operator follow-up, their Project Manager record marked `state=FAILED` (`in_progress=False`) by the escalator — a status, not a rollback. The requested change itself only reaches the record at finalize, so a failed run never advertises a change the engine did not make.
