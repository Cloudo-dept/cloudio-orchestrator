...
- Exactly-once external side effects come from **server-side idempotency keys** `(run_id, step)`, stable across re-drives and retries, independent of any transport. The engine trigger is the one exception: its key is attempt-scoped, so a retry starts a fresh engine run instead of re-attaching to the failed one.
